...
<div class="container">
    <script type="text/javascript">
        ...
        function searchByEmployeeId() {
            var employeeIdInput = $("#employeeIdInSearchForm").val();
            if (employeeIdInput == null || employeeIdInput == undefined || employeeIdInput.trim() == '') {
                employeeIdInput = "0";
            }
            $("#searchForm").attr("action","/admin/project/list/"+employeeIdInput+"/1");
            $("#searchButton").submit();
        }
    </script>
                    ...
                    <div class="col-md-12">
                        <form role="form" class="form-inline" id="searchForm">
                            <div class="form-group">
                                <label for="employeeIdInSearchForm"  class="sr-only" >
                                    工号
                                </label>
                                <input class="form-control    validate[custom[integer]]" id="employeeIdInSearchForm" type="text" placeholder="工号">
                            </div>
                            <button type="submit" id = "searchButton" class="btn btn-default" onclick="searchByEmployeeId()">
                                搜索
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="col-md-12">
                        <table class="table">
                            ...
                            <c:forEach var="item" items="${projectBeanList}" varStatus="status">
                                <tr>
                                    <td>
                                            ${status.index+admin_project_startnum}
                                    </td>
                                    <td>
                                            ${item.floorEmployeeEntity.employeeId}
                                    </td>
                                    ...
                                    <td>
                                        <a data-toggle="modal" data-target="#designerDetailModal" id="designerDetailButton" onclick="showDesignerDetail(${item.floorProjectEntity.id})">查看</a>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu${item.floorProjectEntity.id}">
                                                <li><a href="javascript:top.location='/admin/project/delete/${item.floorProjectEntity.id}'">
                                        ...
                                        <%--<div class="btn-group">
                                            <button class="btn btn-default" type="button" onclick="javascript:top.location='/admin/project/delete/${item.floorProjectEntity.id}'">
                                                <em class="glyphicon glyphicon-remove"></em>
                                                删除
                                            </button>
...
<script>
    ...
    laypage({
        cont: "page",
        pages: ${admin_project_pages}, //可以叫服务端把总页数放在某一个隐藏域，再获取。假设我们获取到的是18
        curr: ${admin_project_currentPage},
        jump: function (e, first) { //触发分页后的回调
            if (!first) { //一定要加此判断，否则初始时会无限刷新
                location.href = '/admin/project/list/'+${admin_employeeIdNow} +'/' +e.curr;
            }
        },
        skin: 'molv', //皮肤
        first: 1, //将首页显示为数字1,。若不显示，设置false即可
        last: ${admin_project_pages}, //将尾页显示为总页数。若不显示，设置false即可
        prev: '<', //若不显示，设置false即可
        next: '>' //若不显示，设置false即可
    });
</script>
...
<div class="modal fade" id="projectUpdateModal" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="projectUpdateForm" action="/admin/project/update"
                      method="post">
                    ...
                    <button type="submit" class="btn btn-default" onclick="updateAndSubmit()">提交</button>
